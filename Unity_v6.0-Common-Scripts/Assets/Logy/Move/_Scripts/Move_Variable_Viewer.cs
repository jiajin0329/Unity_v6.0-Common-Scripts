...
{
    [Serializable]
    public class Move_Variable_Viewer : VariableViewer
    {
        private VariableUI _move_vector2_variable_ui;
        private VariableUI _speed_ratio_variable_ui;
        private VariableUI _move_radian_variable_ui;
        ...
        {
            _move_vector2_variable_ui.Initialize($"{nameof(Move_Model.velocity)} : ", _variableText);
            _speed_ratio_variable_ui.Initialize($"{nameof(Move_Model.speed_ratio)} : ", _variableText);
            _move_radian_variable_ui.Initialize($"{nameof(Move_Model.move_radian)} : ", _variableText);

            _move_image_ui = UnityEngine.Object.Instantiate(_move_image_ui_prefab, _variableText.transform.parent);
        ...
        {
            _move_vector2_variable_ui.UpdateText(_data.move_model.velocity.ToString());
